...
class DataStore with ChangeNotifier {
  ...
  Paywall get standardPaywall {
    if (inReview) {
      ...
      try {
        final PaywallSession session =
            _sessions.firstWhere((session) => session.count == sessionCount);
        try {
          final Paywall paywall = _paywalls
              .firstWhere((paywall) => paywall.id == session.paywallId);
          return paywall;
        } catch (_) {
          try {
            final Paywall defaultPaywall =
                _paywalls.firstWhere((paywall) => paywall.isDefault == true);
